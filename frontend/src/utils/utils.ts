...
export function getErrorMessage(error) {
  ...
  if (error?.response) {
    ...
    switch (response?.status) {
      ...
      default:
        return (response?.data as any)?.message || error.message;
    }
  }
...
export function filterListOrTree<T>(
  ...
  return keywords
    ? dataSource.reduce<T[]>((filtered, d) => {
        ...
        const children = (d as any).children as T[] | undefined;
        let isChildrenMatch: T[] | undefined;
        if (filterLeaf && children?.every(c => (c as any).isLeaf)) {
          isChildrenMatch =
            isMatch || children!.some(c => filterFunc(keywords, c))
              ? children
              : void 0;
        } else {
          isChildrenMatch =
            children &&
            filterListOrTree(children, keywords, filterFunc, filterLeaf);
        }
        if (isMatch || (isChildrenMatch && isChildrenMatch.length > 0)) {
          filtered.push({ ...d, children: isChildrenMatch } as T);
        }
        return filtered;
      }, [])
    : dataSource;
...
export function modelListFormsTreeByTableName(model, type) {
  ...
  model?.forEach(v => {
    const path = v.path;
    if (!path || !Array.isArray(path) || path.length < 1) return;
