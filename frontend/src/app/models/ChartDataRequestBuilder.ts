...
export class ChartDataRequestBuilder {
  ...
  private buildFunctionColumns() {
    // Ensure DateLevelComputedFields are always generated from meta,
    // because in some contexts (e.g. Dashboard, Share) the computedFields
    // may not include them, causing the backend functionColumnMap lookup
    // to fail and producing "Column cannot be resolved" errors.
    const enrichedComputedFields =
      createDateLevelComputedFieldForConfigComputedFields(
        this.dataView.meta,
