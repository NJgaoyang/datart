...
const ValuesOptionsSetter: FC<{
  ...
  const getViewOption = useCallback(async (viewId: string) => {
    ...
    const option: DefaultOptionType[] = meta
      ...
      .map(item => {
        return {
          value: item.name,
          label: getFieldDisplayName(item),
        };
      });
  ...
  const getViewData = useCallback(
    async (value: string[], type?) => {
      // 切换字段时无需重新加载视图字段选项（options 未变），避免英文闪现
      if (type !== 'viewField') {
      ...
      if (type !== 'view') {
        ...
        // 切换字段时复用 viewMap 中已缓存的视图数据
        const dataView = viewMap[viewId];
  ...
  const onViewFieldChange = useCallback(
    async (value: string[], type?) => {
      setOptionValues([]);
      setTargetKeys([]);
      // 仅在切换视图时清空字段选项，切换字段时保留已有选项避免英文闪现
      if (type !== 'viewField') {
        setLabelOptions([]);
