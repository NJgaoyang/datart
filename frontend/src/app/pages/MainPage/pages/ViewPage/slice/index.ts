...
import { DatabaseSchema, Schema, ViewState, ViewViewModel } from './types';
...
const slice = createSlice({
  ...
  reducers: {
    ...
    clearSourceDatabaseSchema(state, action: PayloadAction<string>) {
      delete state.sourceDatabaseSchema[action.payload];
    },
  },
  extraReducers: builder => {
    ...
    builder.addCase(runSql.fulfilled, (state, action) => {
      ...
      if (currentEditingView && action.payload && action.payload.rows) {
        ...
        // Enrich model columns with comment from database schema
        const sourceId = currentEditingView.sourceId;
        if (sourceId) {
          const schemas: DatabaseSchema[] | undefined =
            state.sourceDatabaseSchema[sourceId];
          if (schemas) {
