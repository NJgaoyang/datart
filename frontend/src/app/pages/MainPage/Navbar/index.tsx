...
export function Navbar() {
  ...
  const subNavs = useMemo(
    () => [
      ...
      {
        name: 'auditLog',
        title: t('subNavs.auditLog.title'),
        icon: <AuditOutlined />,
        module: ResourceTypes.Manager,
      },
    ],
  ...
  return (
    <>
      <MainNav>
        ...
        <Nav>
          {navs
            .filter(({ module }) => !hideInNav.includes(module))
            .map(({ name, title, icon, isActive, module }) => {
      ...
      {showSubNav && (
        <SubNav>
          <List
            dataSource={subNavs.filter(
              ({ module }) => !hideInNav.includes(module),
            )}
            renderItem={({ name, title, icon }) => (
              <Tooltip
                ...
                placement="right"
              >
                <SubNavTitle to={`/organizations/${orgId}/${name}`}>
