...
export function getColumnRenderOriginName(c?: ChartDataSectionField) {
  ...
  if (c.aggregate) {
    return `${c.aggregate}(${displayName})`;
  }
...
export function transformMeta(model?: string) {
  ...
  const hierarchyKeys = Object.keys(
    'hierarchy' in jsonObj && Object.keys(jsonObj.hierarchy || {}).length
      ? jsonObj.hierarchy
...
export function transformHierarchyMeta(model?: string): ChartDataViewMeta[] {
  ...
  Object.entries(modelObj?.columns || {}).forEach(
    ([key, column]: [string, any]) => {
      const names = [
        key,
        ...(Array.isArray(column?.name)
          ? [column.name.join('.'), column.name[column.name.length - 1]]
          : [column?.name]),
      ].filter(Boolean);
      names.forEach(name => {
        const metadataKey = String(name);
        const existing = columnMetadata.get(metadataKey);
        if (!columnMetadata.has(metadataKey)) {
          columnMetadata.set(metadataKey, column);
        } else if (existing !== column) {
          columnMetadata.set(metadataKey, undefined);
        }
      });
    },
  );
...
function getMeta(key, column, columnMetadata = new Map<string, any>()) {
  ...
  const metadata = metadataKeys
    .filter(Boolean)
    .map(name => columnMetadata.get(String(name)))
    .find(Boolean);
  if (!isEmptyArray(column?.children)) {
    isHierarchy = true;
...
// 兼容 impala 聚合函数小写问题，将小写的聚合函数名转换为大写
