...
public class DashboardServiceImpl extends BaseService implements DashboardService {
    ...
    public boolean safeDelete(String id) {
        return dashboardMapper.countActiveStorypageReferences(id) == 0;
    }

    @Override
    @Transactional
    public boolean delete(String id, boolean archive) {
        List<String> storyboardNames = dashboardMapper.getActiveStoryboardNamesByDashboard(id);
        if (CollectionUtils.isNotEmpty(storyboardNames)) {
            Exceptions.tr(BaseException.class, "message.delete.error.relation.detail",
                    String.join("、", storyboardNames));
        }
        return delete(id, archive, false);
    }
