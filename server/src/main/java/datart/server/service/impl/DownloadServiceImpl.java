...
public class DownloadServiceImpl extends BaseService implements DownloadService {
    ...
    @Override
    public void syncDownload(DownloadCreateParam downloadCreateParam, HttpServletResponse response) throws Exception {
        ...
        if (null == downloadCreateParam.getDownloadType()) {
            downloadCreateParam.setDownloadType(AttachmentType.EXCEL);
        }

        // Synchronous file generation (same logic as async task but inline)
        AttachmentService attachmentService = AttachmentService.matchAttachmentService(downloadCreateParam.getDownloadType());
