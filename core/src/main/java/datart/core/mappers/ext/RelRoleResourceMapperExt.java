...
public interface RelRoleResourceMapperExt extends RelRoleResourceMapper {
    ...
    @Select({
            "SELECT * FROM rel_role_resource WHERE role_id=#{roleId} AND resource_id=#{resourceId} AND resource_type=#{resourceType} LIMIT 1"
    })
    RelRoleResource selectByUniqueKey(@Param("roleId") String roleId, @Param("resourceId") String resourceId, @Param("resourceType") String resourceType);
}
